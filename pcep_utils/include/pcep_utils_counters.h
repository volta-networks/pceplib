 ...
 *  }
 *
 * To create the above structure of groups, sub-groups, and counters, do the following:
 *
 * struct counters_subgroup *rx_subgroup = create_counters_subgroup("rx counters", 3);
 * struct counters_subgroup *tx_subgroup = create_counters_subgroup("tx counters", 3);
 * struct counters_subgroup *events_subgroup = create_counters_subgroup("events counters", 4);
 *
 ...
 *
 * struct counters_group *cntrs_group = create_counters_group("PCEP Counters", 3);
 * add_counters_subgroup(cntrs_group, rx_subgroup);
...
struct counters_group
{
    ...
    time_t start_time;
    /* Array  of (struct counters_subgroup *) allocated when the group is created.
     * The subgroup is indexed by the (struct counters_subgroup *)->subgroup_id */
    struct counters_subgroup **subgroups;
};

/*
 * Create a counters group with the given group_name and number of subgroups.
 * Subgroup_ids are 0-based, so take that into account when setting max_subgroups.
 * Return true on success or false if group_name is NULL or max_subgroups >= MAX_COUNTER_GROUPS.
 */
struct counters_group *create_counters_group(const char *group_name, uint16_t max_subgroups);

/*
 * Create a counters subgroup with the given subgroup_name, subgroup_id and number of counters.
 * The subgroup_id is 0-based. counter_ids are 0-based, so take that into account when setting max_counters.
 * Return true on success or false if subgroup_name is NULL, subgroup_id >= MAX_COUNTER_GROUPS,
 * or max_counters >= MAX_COUNTERS.
 */
 ...
 * This is useful for RX and TX counters: just create the RX counters and clone it for the TX counters.
 */
 ...
 * Return true on success or false if subgroup is NULL, counter_id >= MAX_COUNTERS, or if counter_name is NULL.
 */
bool create_subgroup_counter(struct counters_subgroup *subgroup, uint32_t counter_id, const char *counter_name);

/*
 * Delete the counters_group and recursively delete all subgroups and their counters.
 ...
 * Return true on success or false if group is NULL, subgroup_id >= MAX_COUNTER_GROUPS,
 * or counter_id >= MAX_COUNTERS.
 */
 ...
 * Return true on success or false if subgroup is NULL or counter_id >= MAX_COUNTERS.
 */
