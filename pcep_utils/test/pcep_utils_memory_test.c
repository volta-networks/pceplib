...
void* test_pceplib_calloc(void *mem_type, size_t count, size_t size)
{
...
void verify_memory_type(struct pceplib_memory_type *mt, uint32_t num_alloc, uint32_t alloc_bytes, uint32_t num_free, uint32_t free_bytes)
{
...
void verify_ext_memory_type(void *mt, int num_malloc_calls, int num_calloc_calls, int num_realloc_calls, int num_strdup_calls, int num_free_calls)
{
...
void test_memory_internal_impl()
{
    ...
    verify_memory_type(pceplib_infra_ptr, alloc_counter, alloc_size, free_counter++, 0);

    /* Test calloc() */
    ptr = pceplib_calloc(PCEPLIB_INFRA, 1, alloc_size);
    CU_ASSERT_PTR_NOT_NULL(ptr);
    pceplib_free(PCEPLIB_INFRA, ptr);
    alloc_counter++;
    verify_memory_type(pceplib_infra_ptr, alloc_counter, alloc_size*alloc_counter, free_counter++, 0);
    ...
    alloc_counter += 2;
    free_counter++;
    verify_memory_type(pceplib_infra_ptr, alloc_counter, (alloc_size*alloc_counter) - 1, free_counter, 0);
...
void test_memory_external_impl()
{
    ...
    /* Test calloc() */
    ptr = pceplib_calloc(PCEPLIB_MESSAGES, 1, alloc_size);
    CU_ASSERT_PTR_NOT_NULL(ptr);
    pceplib_free(PCEPLIB_MESSAGES, ptr);
