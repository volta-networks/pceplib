...
void test_create_counters_group()
{
    ...
    struct counters_group *group = create_counters_group(NULL, num_subgroups);
...
void test_create_counters_subgroup()
{
    ...
    struct counters_subgroup *subgroup = create_counters_subgroup(NULL, subgroup_id, num_counters);
    CU_ASSERT_PTR_NULL(subgroup);

    subgroup = create_counters_subgroup(subgroup_name, MAX_COUNTER_GROUPS + 1, num_counters);
    CU_ASSERT_PTR_NULL(subgroup);

    subgroup = create_counters_subgroup(subgroup_name, subgroup_id, MAX_COUNTERS + 1);
    CU_ASSERT_PTR_NULL(subgroup);

    subgroup = create_counters_subgroup(subgroup_name, subgroup_id, num_counters);
    ...
    CU_ASSERT_EQUAL(strcmp(subgroup->counters_subgroup_name, subgroup_name), 0);
...
void test_create_subgroup_counter()
{
    ...
    CU_ASSERT_FALSE(create_subgroup_counter(subgroup, counter_id + 1, counter_name));
    CU_ASSERT_FALSE(create_subgroup_counter(subgroup, counter_id, NULL));
    CU_ASSERT_EQUAL(subgroup->num_counters, 0);
    CU_ASSERT_TRUE(create_subgroup_counter(subgroup, counter_id, counter_name));
...
{
    struct counters_subgroup *subgroup = create_counters_subgroup("subgroup", 1, 1);
...
void test_reset_group_counters()
{
    ...
    struct counters_subgroup *subgroup = create_counters_subgroup("subgroup", subgroup_id, 10);
...
{
    uint16_t counter_id = 1;
    struct counters_subgroup *subgroup = create_counters_subgroup("subgroup", 1, 10);
    create_subgroup_counter(subgroup, counter_id, "counter");
