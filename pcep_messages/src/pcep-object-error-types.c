...
/* Store how many Error Values each Error Type has */
static const int error_value_lengths[] =
{
    0,  8,  0,  2,  9,  /* Types 0 - 4 */
    8, 15,  0,  0,  0,
    32, 0,  3,  1,  0,  /* Types 10 - 14 */
    2,  2,  4,  4, 14,
    7,  2,  0,  2,  3,  /* Types 20 - 24 */
    5, 11,  3,  2,  4
};
...
const char *get_error_type_str(enum pcep_error_type error_type)
{
    ...
    {
        pcep_log(LOG_DEBUG, "get_error_type_str: error_type [%d] out of range [0..%d]",
                error_type, MAX_ERROR_TYPE);
...
const char *get_error_value_str(
        enum pcep_error_type error_type, enum pcep_error_value error_value)
{
    if (error_type < 0 || error_type >= MAX_ERROR_TYPE)
    {
        pcep_log(LOG_DEBUG, "get_error_value_str: error_type [%d] out of range [0..%d]",
                error_type, MAX_ERROR_TYPE);
    ...
    {
        pcep_log(LOG_DEBUG, "get_error_value_str: error_value [%d] out of range [0..%d]",
                error_value, MAX_ERROR_VALUE);

        return NULL;
    }

    if (error_value > error_value_lengths[error_type])
    {
        return "Unassigned";
    }
