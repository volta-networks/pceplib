...
pcep_msg_create_open(uint8_t keepalive, uint8_t deadtimer, uint8_t sid)
{
    ...
    dll_append(message->obj_list, pcep_obj_create_open(keepalive, deadtimer, sid, NULL));
...
{
    struct pcep_message *message = pcep_msg_create_common(PCEP_TYPE_OPEN);
    dll_append(message->obj_list, pcep_obj_create_open(keepalive, deadtimer, sid, tlv_list));
...
struct pcep_message*
pcep_msg_create_request(struct pcep_object_rp *rp,  struct pcep_object_endpoints_ipv4 *endpoints, double_linked_list *object_list)
{
...
pcep_msg_create_error(uint8_t error_type, uint8_t error_value)
{
    ...
    dll_append(message->obj_list, pcep_obj_create_error(error_type, error_value));

    return message;
}

struct pcep_message*
pcep_msg_create_error_with_objects(uint8_t error_type, uint8_t error_value, double_linked_list *object_list)
{
    struct pcep_message *message = pcep_msg_create_common_with_obj_list(PCEP_TYPE_ERROR, object_list);
    dll_prepend(message->obj_list, pcep_obj_create_error(error_type, error_value));
...
pcep_msg_create_report(double_linked_list *state_report_object_list)
{
    return(state_report_object_list == NULL ?
            NULL :
            pcep_msg_create_common_with_obj_list(PCEP_TYPE_REPORT, state_report_object_list));
}
...
pcep_msg_create_update(double_linked_list *update_request_object_list)
{
    ...
    return(pcep_msg_create_common_with_obj_list(PCEP_TYPE_UPDATE, update_request_object_list));
}
