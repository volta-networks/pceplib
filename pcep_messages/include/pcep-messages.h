...
struct pcep_message_header
{
    ...
    enum pcep_message_types type;  /* Defines message type: OPEN/KEEPALIVE/PCREQ/PCREP/PCNOTF/ERROR/CLOSE */
};
 ...
 * When creating messages, any objects and tlvs passed into these APIs will be free'd
 * when the pcep_message is free'd. That includes the double_linked_list's. So, just
 * create the objects and TLVs, put them in their double_linked_list's, and everything
 * will be managed internally. The message will be deleted by pcep_msg_free_message()
 * or pcep_msg_free_message_list() which, in turn will call one of: pcep_obj_free_object()
