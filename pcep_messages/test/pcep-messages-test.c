...
/*
 * Notice:
 * All of these message Unit Tests encode the created messages by explicitly calling
 * pcep_encode_message() thus testing the message creation and the message encoding.
 */
...
void test_pcep_msg_create_open()
{
    ...
    CU_ASSERT_EQUAL(message->encoded_message_length,
            MESSAGE_HEADER_LENGTH + pcep_object_get_length(PCEP_OBJ_CLASS_OPEN, PCEP_OBJ_TYPE_OPEN));
...
void test_pcep_msg_create_request()
{
    ...
    CU_ASSERT_EQUAL(message->encoded_message_length,
            ...
            pcep_object_get_length_by_hdr(&rp_obj->header) +
            pcep_object_get_length_by_hdr(&ipv4_obj->header) +
            pcep_object_get_length_by_hdr(&bandwidth_obj->header));
    CU_ASSERT_EQUAL(message->msg_header->type, PCEP_TYPE_PCREQ);
    CU_ASSERT_EQUAL(message->msg_header->pcep_version, PCEP_MESSAGE_HEADER_VERSION);
    pcep_msg_free_message(message);
}
...
void test_pcep_msg_create_reply()
{
    ...
    struct pcep_object_ro_subobj *ero_subobj =
            (struct pcep_object_ro_subobj *) pcep_obj_create_ro_subobj_32label(true, 1, 10);
    ...
    CU_ASSERT_EQUAL(message->encoded_message_length,
            ...
            OBJECT_RO_SUBOBJ_HEADER_LENGTH + 6 /* size of the 32label */);
    CU_ASSERT_EQUAL(message->msg_header->type, PCEP_TYPE_PCREP);
    CU_ASSERT_EQUAL(message->msg_header->pcep_version, PCEP_MESSAGE_HEADER_VERSION);
    pcep_msg_free_message(message);
}
...
void test_pcep_msg_create_close()
{
    ...
    CU_ASSERT_EQUAL(message->encoded_message_length,
            MESSAGE_HEADER_LENGTH +
            pcep_object_get_length(PCEP_OBJ_CLASS_CLOSE, PCEP_OBJ_TYPE_CLOSE));
...
void test_pcep_msg_create_error()
{
    ...
    CU_ASSERT_EQUAL(message->encoded_message_length,
            MESSAGE_HEADER_LENGTH +
            pcep_object_get_length(PCEP_OBJ_CLASS_ERROR, PCEP_OBJ_TYPE_ERROR));
...
void test_pcep_msg_create_report()
{
    ...
    CU_ASSERT_EQUAL(message->obj_list->num_entries, 1);
    CU_ASSERT_EQUAL(message->encoded_message_length,
            MESSAGE_HEADER_LENGTH + lsp->header.encoded_object_length);
